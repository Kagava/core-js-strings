 ...
 *   repeatString('abc', -2) => ''
 */
function repeatString(value, mul) {
  return mul >= 0 ? value.repeat(mul) : '';
}
...
function removeFirstOccurrences(value, sub) {
  //  indexOf
 ...
 */
function removeLastOccurrences(value, sub) {
  const index = value.lastIndexOf(sub);
  if (index === -1) {
    return value;
  }
  return value.slice(0, index) + value.slice(index + sub.length);
}
...
function formatTime(min, sec) {
  // return `${min.toString().padStart(2, '0')}:${sec.toString().padStart(2, '0')}`; xDDDDDDD
 ...
 */
function containsSubstring(string, substr) {
  //  return string.indexOf(substr) !== -1 ? 1 : 0;
...
function countVowels(string) {
  ...
  string.split('').forEach((element) => {
    if (volwels.includes(element)) count += 1;
  });
  return count;
}
...
function isPalindrome(string) {
  const upperString = string.replace(/\W|_/g, '').toUpperCase();

  const toPalindorme = upperString.split('').reverse().join('');

  return toPalindorme === upperString;
}
...
function findLongestWord(string) {
  ...
  return indexMax !== -1 ? stringArray[indexMax] : '';
}
