...
function removeFirstOccurrences(str, value) {
  const firstS = str.indexOf(value);
  if (firstS === -1) return str;
  const resultString = str.slice(0, firstS) + str.slice(firstS + value.length);
  return resultString;
}
...
function removeLastOccurrences(str, value) {
  const firstS = str.lastIndexOf(value);
  if (firstS === -1) return str;
  const resultString = str.slice(0, firstS) + str.slice(firstS + value.length);
  return resultString;
}
...
//     let tempI = str.indexOf(vowels[i]);
//     while (tempI !== -1) {
//       sum += 1;
//       tempI = str.indexOf(vowels[i], tempI + 1);
//       console.log(tempI);
//     }
//   }
// }
...
function isPalindrome(str) {
  const solidUpperStr = str.replace(/\W/g, '').toUpperCase();
  const revStr = reverseString(solidUpperStr);
  const booly = revStr.localeCompare(solidUpperStr);
  if (!booly) return 1;
  return 0;
}
...
function reverseWords(str) {
  ...
  const resultArr = strArray.map((item) => reverseString(item));
...
/* 
  const newStr = '';
  for (let i = 0; i < str.length; i += 1) {
    newStr += str[i] === str[i].toLowerCase() ? str[i].toUpperCase
  } */
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (alphabetUpper.indexOf(str[i]) !== -1) {
      resultStr +=
        alphabetUpper[
          (alphabetUpper.indexOf(str[i]) + 13) % alphabetUpper.length
        ];
    } else if (alphabetLower.indexOf(str[i]) !== -1) {
      resultStr +=
        alphabetLower[
          (alphabetLower.indexOf(str[i]) + 13) % alphabetLower.length
        ];
    } else {
      resultStr += str[i];
    }
...
function getCardId(value) {
  ...
  let id = card.indexOf(value.at(-1)) * 13;
